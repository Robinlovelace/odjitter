...
We find that route networks generated from jittered desire lines are more geographically diffuse that route networks generated from 'unjittered' (centroid based) desire lines.
The approach, implemented in the Rust crate odjitter, is a computationally efficient and flexible way to simulate diffuse travel patterns from OD data.
...
The 'jittering' approach presented in this paper allows the user to adjust start and end locations based on variables such as transport network density, residential density or size of commercial buildings acting as trip attractors.
...
```{r jitters, fig.cap="Illustration of jittering and disaggregation of origin-destination (OD) data with a minimal input dataset. Subfigure A) shows the conventional way of representing OD data as desire lines between zone centroids. Subfigures B) and C) show the same desire lines but with jittered origin and start locations based on simple random sampling of points and sampling locations on the road network. Subfigure D) shows the combined impact of disaggregation and jittering.", out.width="80%"}
set.seed(2021)
fn = c(
...
Full disaggregation (not demonstrated in this paper) would lead to one route being calculated for every trip; this would increase increase computational requirements but could generate outputs that could feed into agent based models.
...
- What additional refinements to the jittering techniques outline in this paper could generate more realistic disaggregated OD pairs, e.g. as an input into agent based models (ABM)?
...
Before further refinements are made, the priority should be validation to answer the first of these questions.
