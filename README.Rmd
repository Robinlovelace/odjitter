...
file.copy("README.pdf", "jittering-paper.pdf", TRUE)
piggyback::pb_upload("jittering-paper.pdf")
...
Origin-destination (OD) data is a vital source of information on travel patterns but its utility is limited by reliance on zone centroids.
This paper presents a reproducible and open two-stage 'jittering' approach to tackling this problem, which (1) uses random points to represent unique start and end points (sampling), and (2) splits OD pairs representing many trips into many 'sub-OD' pairs.
We find that route networks generated from jittered OD data are more diffuse and potentially realistic based on an example from Edinburgh.
Further work is needed to validate the approach and to find optimal parameters for sampling and disaggregation.

# Questions

Origin-destination (OD) datasets are ubiquitous in transport planning to efficiently represent aggregate travel behavior.
...
1. Centroid to centroid representations, involving aggregating all "trip ends [destinations, and origins] into zone centroids" [@leurent_stochastic_2011]
2. Subdividing zones into subzones and then taking subzone centroids as trip origins and destinations [refs]
3. Creation of 'centroid connectors', also referred to as links "between trip ends and zonal anchors" [@leurent_stochastic_2011]

In this paper we present a new approach to pre-processing OD datasets which combines OD data disggregation [refs] *and* randomisation of start and end locations [refs] into a single technique.
The 'jittering' framework we present is flexible, allowing the user to specify the level of disaggregation required for their applications, appropriate start points and trip attractors, and weights highlighting the relative importance of different origin and destination points.

Unlike OD disaggregation processes outlined in previous papers cited the *output* of the jittering process outlined in this paper is *entirely new OD pairs* which can be visualised as desire lines and passed to the network assignment stage of transport modelling workflows (routing).
We refer to the approach as jittering, noting the use term to describe a similar process of adding "random noise to the data" for data visualization [wickham_2016a_ggplot2].
...
m4 = tm_shape(zones, bbox = bbox) +
  ...
  tm_lines("foot", palette = "Set1", lwd = 6, breaks = c(75, 100, 125, 150), title.col = "Walking trips per day") +
...
We found that jittering generates desire lines and routes that are more diverse and potentially realistic than the common default of using zone centroids to represent start and end points.
...
The results of applying the approach to a large OD datasest, representing travel to work in Edinburgh, with sampling on the transport network, are illustrated in Figure \@ref(fig:rneted) (right).
The 'jittered network' is substantially more dense and presumably more realistic than the 'unjittered network', a gain in network fidelity that comes at low computational cost.

```{r}
od_sf_central = odsf %>% 
  filter(geo_code1 %in% zones$InterZone) %>% 
  filter(geo_code2 %in% zones$InterZone) 
od_sf_100 = od_sf_central %>%
  top_n(n = 500, wt = foot)

  # filter(foot > 50) 
# sum(od_sf_100$foot) / sum(od_sf_central$foot) # 80%
# qtm(zones) +
# tm_shape(od_sf_100) +
#   tm_lines() 
bbox = tmaptools::bb(od_sf_100, ext = 1.2)

od_sf_jittered = od::od_jitter(od_sf_100, z = zones)
od_sf_road = od::od_jitter(od_sf_100, z = zones, road_network_buffer, max_per_od = 1000)
od_sf_disaggregated = od::od_jitter(od_sf_100, z = zones, road_network_buffer, max_per_od = 200)
# od_sf_disaggregated = od::od_jitter(od_sf_100, z = zones, road_network_min, max_per_od = 200, population_column = "foot")
m1 =  tm_shape(zones, bbox = bbox) +
  tm_borders(col = "grey") +
  tm_shape(od_sf_100, bbox = bbox) +
  tm_lines(lwd = "foot", scale = 3, title.lwd = "Walking trips per day") +
...
```{r, eval=FALSE}
routes_od = route(l = od_sf_100, route_fun = route_osrm, osrm.profile = "foot")
...
fn = c(
  "Centroid origins and destinations",
  "Jittered origins and destinations (simple random sampling)",
  "Jittered origins and destinations (sampling on transport network)",
  "Jittered origins and destinations (on network with disaggregation)"
)
i = 1
...
rnets$foot[rnets$foot < 10] = 10
tm_shape(rnets, bbox = tmaptools::bb(rnets, 0.5)) +
  tm_lines(lwd = "foot", scale = 15, lwd.legend = c(500, 5000), title.lwd = "Walking trips per day") +
...
This is however, to the best of our knowledge, the first paper focussed on the two step process of jittering described in this paper --- sampling origin and destination points (with simple random sampling or by sampling from the nodes on the network) and disagreggation --- supported with a reproducible implementation based on open source software (see accompanying code).
...
- Which jittering settings (including sampling strategies and levels of disaggregation) represent the best 'boom for buck' in terms of network accuracy divided by computational requirements?
- And can further refinements, for example sampling with weights to increase the proportion of trips associated with large buildings or certain road types, yield further benefits? 
