...
This paper presents a 'jittering' approach to tackling this problem by (1) sampling unique origin and destination locations for each OD pair, and (2) splitting 'large' OD pairs representing many trips into 'sub-OD' pairs.
We find that route networks generated from jittered OD data are more diffuse, based on an example dataset representing walking trips.
