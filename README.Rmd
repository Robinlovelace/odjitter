...
This is not the first time that such methods of "centroid connector placement" have been developed [@jafari_investigation_2015].
It is, to the best of our knowledge, the first time that the two step process of jittering described in this paper --- sampling origin and destination points (with simple random sampling or by sampling from the nodes on the network) and disagreggation --- has been documented and implemented with open, reproducible code (see the code that accompanies this paper).

There are many ways of achieving jittered OD pairs, and the one introduced here can be further refined using, for instance, different weights for zones activities or building categories, of for road hierarchy. Fixed destinations with randomized origins (such for school trip problems) can also be refined.
