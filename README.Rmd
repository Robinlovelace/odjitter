...
1. Centroid to centroid representations, involving the simplifying assumption that all trip destinations and origins can be represented by zone centroids [@leurent_stochastic_2011]
2. Subdividing zones at which data is available to subzones [@opie_commodityspecific_2009]
...
The 'jittering' framework we present is flexible, enabling the user to adjust the level of disaggregation required for their applications and start and end points from which disaggregate OD pairs are sampled.
...
Unlike afforementioned papers, jittering can be understood as a pre-processing stage that can add value to OD datasets when representing the diffuse nature of travel networks is important, something that is particularly important when designing for active travel [@buehler_bikeway_2016].
Jittering results in desire lines which can then be sent to the network assignment stage of transport modelling workflows (routing).
