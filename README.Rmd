...
# create diff
system("latexdiff jittering-paper-resubmission.tex README.tex > diff-resubmission2.tex")
tinytex::pdflatex("diff-resubmission2.tex")
browseURL("diff-resubmission2.pdf")
piggyback::pb_upload("diff-resubmission2.pdf")
piggyback::pb_download_url("diff-resubmission2.pdf")

file.copy("odjitter_responses/README.pdf", "responses2.pdf", TRUE)
piggyback::pb_upload("responses2.pdf")
piggyback::pb_download_url("responses2.pdf")
...
Reproducible findings show that route networks generated from jittered desire lines are more geographically diffuse than route networks generated from 'unjittered' (centroid based) desire lines.
The approach, implemented in the open source Rust crate odjitter, is a computationally efficient and flexible way to simulate diffuse travel patterns from OD data.
...
In this paper we present a new approach to pre-processing OD datasets with a focus route network generation.
Unlike established approaches which convert centroid based desire lines to routes and then route networks [@morgan_travel_2020], the 'jittering' approach presented in this paper allows the user to adjust start and end locations based on variables such as transport network density, residential density or size of commercial buildings acting as trip attractors.
...
The approach is flexible, enabling the user to adjust the level of disaggregation, the location of start and end points from which disaggregate OD pairs are sampled, and weights representing the importance of different trip 'originators' and 'attractors'.
...
We developed the approach to support strategic investment in walking and cycling infrastructure networks based on our observation that route networks resulting from OD pairs from and to single centroids per zone were too sparse, and not adequate for active travel representation.
The approach can 'slot into' existing transport modelling workflows that use desire lines as the basis of route network assignment, uptake modelling, and route network generation workflows [@morgan_travel_2020].
...
The key elements of the jittering approach outlined in this paper are described below.
...
We found that jittering generates desire lines that are more geographically diffuse than desire lines resulting from the common method of using zone centroids to represent start and end points.
This shown in Figure \@ref(fig:jittered514), which demonstrates the use of simple random sampling and sampling nodes on transport networks with reference to a real world example.
...
The figure shows that the 'jittered networks' are more diffuse than the 'unjittered network'.
Disaggregation leads to more diffuse networks as shown in Figure \@ref(fig:rneted) (D).

Advantages of this approach include simplicity, low computational cost and flexibility: the level of disaggregation can be adjusted depending on computational considerations.
Full disaggregation (not demonstrated in this paper) would lead to one route being calculated for every trip; this would increase increase computational requirements but could generate outputs that could feed into agent based models (ABM) such as A/B Street and MATSim [@abstreet_2022; @horni_multiagent_2016].
The approach has fewer data input requirements than established methods and could be used in setting where sub-zones are unavailable.
...
This is however, to the best of our knowledge, the first paper focussed on the two step process of jittering described in this paper --- sampling origin and destination points (with simple random sampling or by sampling from the nodes on the network) and disagreggation --- supported with a reproducible implementation based on open source software: the jittering methods presented in this paper are implemented in the R package [`od`](https://itsleeds.github.io/od/).
A more feature complete and high performance implementation created in parallel with this paper is the Rust crate [`odjitter`](https://github.com/dabreegster/odjitter).
Code accompanying this paper which reproduces the results in each language.
...
- What additional refinements to the jittering techniques outline in this paper could generate more realistic disaggregated OD pairs, e.g. as an input into ABMs?
